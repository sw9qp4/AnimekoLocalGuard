...
**最后更新：** 2026-09-20 00:40（本地时间）
**当前阶段：** G0 `BUILT` 且上游 Gradle 测试任务可用；**G1 五项交付全部落地并 `BUILT`**；
两个真实文字显示入口均已接入；剧情包格式/装载校验/生成工具/对齐生成/端到端测试均已就绪
｜纯逻辑 `UNIT_TESTED`（**308/308**，19 个类）
｜界面层 **BUILT**（`testAndroidHostTest` 通过，含列表重算与状态刷新修复）
｜**外部审阅已回应：H-1~H-5 已修 + H-6 列表侧已修；H-6 覆盖层侧 / H-7 / H-8 未修**
｜**素材可用性已实测**（`docs/MATERIAL_SURVEY.md`）：带时间字幕需外部提供、自有弹幕源覆盖率低
｜**短片片单已收敛并逐部实测**（`docs/SHORT_SERIES_MEASUREMENT.md`）：85 部 ≤14 集候选
｜**未做：真实模型、真实作品资料（需用户提供）、真机验证、任何性能测量**
｜**当前打开开关会屏蔽全部弹幕**（无分析能力 → 未审核内容不显示），不能用于正常观看
...
## 〇之十五、本轮（2026-09-19 第二十二轮）进展：修复 H-6（列表不重算）+ 同类状态行缺陷

上一轮审阅留下的三项里，H-6 被列为最优先。本轮修掉它的**列表侧**，
并顺带发现、修掉一个同类的状态行缺陷。覆盖层侧**未修**，原因写在下面。

### 1. H-6 列表侧：配置与进度现在是**输入**，不再只是"读取"
...
val allDanmakuListFlow = combine(
    ...
    danmakuRepository.selfId,          // ← 只有这两个输入
) { danmakuList, selfId ->
    if (!localGuardConfigSource.current.enabled) ...   // ← 读配置，但不是输入
    else { val position = player.currentPositionMillis.value ... }  // ← 读位置，但不是输入
}
```

**读取一个值不等于依赖它。** 开关、档位、播放位置都不在输入里，因此列表算过一次就不再更新。

修法三点：

1. **配置成为输入**：新增 `localGuardConfig: StateFlow<GuardUserConfig>` 并入 `combine`。
   配置**必须**有自己的输入——用户可以在**暂停时**打开开关，那时永远不会来位置采样。
2. **位置成为输入**：用上游已有的 `sampleWithInitial`（本项目此前用它做 OP/ED 跳过）
   以 **5 秒**粒度采样。列表的可见集合是位置的**阶跃函数**，不需要逐帧重算。
3. **判定用真实位置**：采样只决定*何时*重算，不决定*答案*。

**5 秒这个数字是刻意的取舍**：各档余量是 0/15/30 秒，所以新解锁的条目最多**晚 5 秒**出现；
但**绝不会提前**，因为判定用的始终是真实当前位置。晚是安全方向。

### 2. 顺带修掉一个同类缺陷：状态行不会因资料到达而刷新

`localGuardStatusFlow` 的输入是"配置 + 计数 + 集数"。上一轮修好 H-4（资料会推送到会话）后，
**这三个信号都不会因资料到达而改变** ⇒ 状态行会继续显示"资料缺失"。

**一条谎报资料是否加载的状态行比没有状态行更糟**——用户会去排查一个已经解决的问题。

已新增 `localGuardKnowledgeVersion`（资料实际变化时递增）并纳入状态流。

这属于"修一个接线缺陷时发现另一个同源缺陷"，与本项目历史上那 5 次"实现了但没接线"同类。
...
| 情形 | 是否已覆盖 |
|---|---|
| 正常播放（新事件不断到达） | ✅ 按当前档位与位置判定 |
| 拖动进度（上游重发 `Repopulate`，携带事件自身的 `playTimeMillis`） | ✅ 会重新判定 |
| **弹幕已交给显示层后，用户切换开关或档位** | ❌ **已放行的不追溯隐藏，已拦下的不追溯显示** |

**未修的原因**：修复需要在上游侧触发一次重新填充（`requestRepopulate` 之类的路径，
本轮**未核实具体 API**），那会在播放中插入一次行为变化。
**没有设备时我无法判断它会不会造成画面跳动或闪烁**，所以先记录、不猜着改。

### 4. 本轮验证结果（实测）

| 验证 | 结果 |
|---|---|
| `:app:shared:testAndroidHostTest` | **BUILD SUCCESSFUL**，7m11s |

本轮首次遇到并解决一个真实编译错误：`localGuardStatusFlow` 引用了在**文件中更靠后**声明的
`localGuardConfig` / `localGuardKnowledgeVersion`，Kotlin 的属性初始化顺序不允许。
已把声明前移，并用新增的 `tools/check_decls.js` 复核顺序与声明计数。

新增工具：`tools/check_decls.js`（检查 import 是否齐备、是否有两行粘成一行、声明是否恰好一次）。

### 5. 剩余未修（不当作已完成）

| # | 问题 | 状态 |
|---|---|---|
| H-6 覆盖层 | 切换开关/档位时不追溯重新判定 | 未修，需设备 |
| H-7 | 对齐只按作品绑定；残差未进入运行时误差边界 | 未修，需扩展数据结构 |
| H-8 | 缓存与调度四项 | 未修，接模型前必须解决 |

### 6. 下一轮计划

1. 让 `semanticsProvider` 真正返回结果（规则基线），使"通用分类"这一行能力真正可用
2. 至此开关打开才有意义——目前打开会屏蔽全部弹幕（无分析能力时未审核内容不显示）
3. 之后再重建 → 连接手机验证（含覆盖层那一条）

---

## 〇之十四、上一轮（2026-09-19 第二十一轮）进展：外部审阅回应 + 修复 5 处错误放行缺陷

外部审阅（依据提交 `9f5598d`）给出了详细意见。审阅者指出**四处我没有发现的源码级缺陷**，
其中两处使"资料已加载 / 集数已识别"与"过滤器实际在按什么判断"之间出现缺口。
逐条核对源码后，**四处全部确认成立**，已修复并补测试。

### 1. 已修复的缺陷（H-1 ~ H-5）

**H-1 未核对的事实在跨集后被自动解锁（缺陷）**

`TimelineEvaluator.evaluate` 的分支顺序是"先按剧集顺序判断（跨集直接返回 unlocked），
再检查同集核对状态"，于是核对门槛被绕过：一条 `AI_CANDIDATE` 事实，
**只要播放到它声称的揭晓集之后就会被解锁**。这与本模块自己的文档冲突：
「未核对项不得作为高可信依据自动解锁」。

修复：把核对检查提到**任何分支之前**，成为解锁前置条件。
副作用（刻意）：未核对事实在整个覆盖范围内持续受保护，即使那一集已播完。
两个方向都有代价，一律选择"宁可保守，不可提前解锁"。

**H-2 多事实判定结果依赖事实列表顺序（缺陷）**

原实现取"第一个未解锁事实的原因"决定是否降级放行。审阅者给出反例，我逐字复算确认：
均衡档下 explicit 阈值 0.60、unknownFactFloor 0.80、分数 0.70 ——
"时间未知 → 未来集"顺序得到 `TimingUnknown` 从而 0.70 < 0.80 **放行**；
"未来集 → 时间未知"顺序得到 `FutureEpisode` 从而**屏蔽**。信息相同，结论相反。

修复：逐项独立判断后合并。任一事实**确定**未解锁即屏蔽（一个"时间未知"不得抵消
一个"已确认属于未来集"）；其余情形取"最严"而非"第一个"。上报原因也改为确定性选择。

**H-3 集数变成未知时沿用上一集（缺陷，本轮最严重）**

`syncLocalGuardEpisode` 在 `episodeNumber == null` 时直接 `return`，**什么都不做**，
于是会话里保留着**上一集**的集数。原注释自称"the session treats 'episode unknown' as
'do not guess' rather than reusing the previous episode"——**这句话是错的**，
会话根本没被通知。注释描述的是意图，不是行为。

危险方向：旧进度可能让**未来事实**被当成"已经揭晓"，从而**放行**。
这比"没有过滤"更糟，因为它看起来像是在工作。

修复：新增 `DanmakuGuardSession.markEpisodeUnknown()`，明确清除集数并递增生成号。

**H-4 剧情包异步加载完成后没有绑定到会话（缺陷）**

会话构造时 `knowledge = null`（构造时资料必然还没加载）；`loadLocalGuardKnowledge()`
只写两个字段；而 `syncLocalGuardEpisode` 只在集数**变化**时触发。资料加载完成时集数
通常没变，因此 `startEpisode` 不会再被调用，会话**永久持有 null 包**。
"文件加载成功" ≠ "过滤真的用上了这个文件"。

修复：新增 `DanmakuGuardSession.updateKnowledge()`（不切集、不重置计数，但清事实关系缓存），
加载完成后立即调用。

**H-5 无审核能力时静默放行（原判断描述不准确，已采纳审阅者的决定）**

重新读总任务说明后发现两条规则分工明确，我原先混为一谈：

| 条款 | 情形 | 要求 |
|---|---|---|
| 第 12 节 | 故障 | 未审核内容**不显示**，明确提示；不得静默恢复原样弹幕 |
| 第 13 节 | 能力降级 | 按三档处理，**仍可运行通用分类**，不标为完整防剧透 |

真实缺陷是：语义解析返回 null（= 这条弹幕**未经审核**）时放行。
原测试名 `noSemanticsIsTreatedAsUnknownNotSafe`（"不当作安全"）**却断言放行**——
名字与断言矛盾。

修复：不再放行，计入 `failed`；保留资料缺失/集序未知的降级路径；
状态行区分"无分析能力"与"分析故障"；`semanticsReady` 改为**基于证据**
（曾成功产出结论且最近未失败），否则会出现"判定在屏蔽、界面说已接入"的最糟组合。

### 2. 修好的 release 包名缺陷（上一轮遗留）

上一轮发现 release 变体包名是上游的 `me.him188.ani`（与官方冲突），改了但没验证。
本轮重建并**从产物核验**：

```
node tools/apk_inspect.js <release.apk> --expect-package me.him188.ani.localguard --expect-assets-story 0
→ "package": "me.him188.ani.localguard"     OK
→ bytes 37,012,169   sha256 8d072274…
```
...
| # | 问题 | 更正 |
|---|---|---|
| H-9 | 用"只删了 5 行"论证上游语义不变 | **无效论证**。新增一行提前返回就足以改变行为。准确说法：上游既有逻辑没有被删除，但新增分支会在开关打开时改变行为 |
| H-10 | 把"缺字幕"说成根本阻塞 | 表述过强。缺字幕只阻塞"分钟级事实判断"；包名/安装核验、规则拦截、通用分类都不需要它 |
| H-11 | 把"自有源抽样低"推广成"实际使用中弹幕少" | 超出证据。准确说法：自有源覆盖率低，聚合源**未测量** |

另外把我关于字幕的表述从"本机不可获取"改为"已检查的接口没有提供"——
后者才是事实。

### 4. 审阅指出但我**尚未修复**的三项（不得当作已完成）

| # | 问题 | 核对结果 |
|---|---|---|
| H-6 | `allDanmakuListFlow` 的 `combine` 输入里**没有**配置流也没有播放位置流，但函数体读取了这两者 ⇒ 开关/档位/进度变化不会触发重新过滤 | **确认成立。** 未修：需先决定重新计算的触发条件与代价，并配合性能测量 |
| H-7 | 对齐只按 `workId` 绑定（不按集、不按视频版本）；90 分位残差不足以证明每个剧情区段都安全 | **部分成立**（路径确认；生成器是否写非零误差待核实）。未修：需扩展数据结构 |
| H-8 | 缓存与调度四项：无事实即跨作品复用、失败进长期缓存、缺 `knowledgeVersion`、提供者仍是同步接口 | **四项均成立**，逐条核对了源码。未修：属接口变更，应在实现规则基线时一并处理 |
...
| 验证 | 结果 |
|---|---|
| 独立运行器 | **308/308 通过，19 个类**（292 → 308；新增 22 例、删除 1 例） |
| 用例数交叉核对 | `tools/test_inventory.js` 扫描 `@Test` 注解，同为 **308** |
| `:app:shared:testAndroidHostTest` | **BUILD SUCCESSFUL**，30m51s（首次编译 ViewModel 改动） |
| `:app:android:assembleRelease` | **BUILD SUCCESSFUL**，13m01s |
| release 包名 | **`me.him188.ani.localguard`** ✅（产物核验通过） |

证据：`docs/evidence/post_review_fix_and_rebuild.txt`、
`docs/audit/H-REVIEW-RESPONSE.md`、`STATE.json → externalReviewRound`。

### 6. 下一轮计划（依审阅意见调整）

审阅者的核心建议是：**不应只"再打两个包"，而应先修复会错误放行的逻辑，
再构建一个真正能验证拦截的版本。** 已按此调整：

1. **修 H-6 / H-7 / H-8**（剩余的正确性缺陷）
2. **做无需真实番剧知识库的最小闭环**：让 `semanticsProvider` 真正返回结果（规则基线），
   用虚构时间线验证"同一内容在揭晓前后变化"，再对**客户端实际收到的一条真实弹幕**
   确认显示前确实被拦截
3. 修复后重新构建 → 连接手机验证
4. 通用模型与剧情数据生产并行推进
...
`docs/audit/H-REVIEW-RESPONSE.md`、`docs/evidence/post_review_fix_and_rebuild.txt`、
`tools/test_inventory.js`、`tools/show_test.js`。

---

## 〇之十三、上一轮（2026-09-19 第二十轮）进展：外部审阅材料 + 发布到 GitHub
