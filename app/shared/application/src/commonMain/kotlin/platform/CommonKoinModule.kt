...
//            subjectPersonRelationDao = database.subjectPersonRelation(),
    ...
    // AnimekoLocalGuard（非官方新增模块）：本地弹幕过滤的配置，与官方设置同一个 DataStore 的独立键。
    single<GuardConfigRepository> {
        GuardConfigRepository(
            getContext().dataStores.preferencesStore,
            coroutineScope.coroutineContext,
        )
    }
    // AnimekoLocalGuard：剧情包与片源对齐的来源。
    // 目录式装载（story/<workId>.json）与文件读取器分开：读取器按平台提供，
    // 解析、版本检查与缓存对所有平台共用一套，避免各平台各写一遍。
    single<StoryKnowledgeSource> {
        TextStoryKnowledgeSource(getStoryPackTextReader(getContext()))
    }
