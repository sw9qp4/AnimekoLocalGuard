    ...
    EpisodeVideoImpl(
        ...
        sideSheets = { sheetsController ->
            EpisodeVideoDefaults.SideSheets(
                ...
                playerSettingsPage = {
                    val localGuardStatus by vm.localGuardStatusFlow.collectAsStateWithLifecycle()
                    // 展开状态由页面持有：设置面板在组合重建（如横竖屏切换）之间会被销毁重建。
                    var showLocalGuardDiagnostics by rememberSaveable { mutableStateOf(false) }
                    EpisodeVideoSideSheets.DanmakuSettingsNavigatorSheet(
                        ...
                        onShowLocalGuardDiagnosticsChange = { showLocalGuardDiagnostics = it },
                    )
                },
